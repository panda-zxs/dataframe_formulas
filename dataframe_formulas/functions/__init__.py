...
from ..exceptions import (BaseError, BroadcastError, FoundError,
                          InvalidRangeError, RangeValueError)
        ...
        if operator == '=':
            ...
            if it:
                ...
                match = re.compile(''.join(
                    sum(zip(map(_, _re_condition.split(condition)),
                            tuple(map(lambda v: '.%s' % v, it)) + ('', )), ()))).match
